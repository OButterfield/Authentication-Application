 ...
 * Orchestrates UserAuthenticationService and SessionService for authentication
 * and session creation. Exception handling is centralized in
...
public class SessionsController implements SessionsApi {
	...
	/**
	 * Creates a new login session by authenticating user credentials. Authenticates
	 * user via UserAuthenticationService and creates session via SessionService.
	 * Stores the session in the user's sessions array and returns session details
	 * including sessionId, profileId, email, and expiryTime. Exceptions are handled
	 * by GlobalExceptionHandler for consistent error responses.
	 *
	 * @param createSessionRequest
	 *            the request containing email and password
	 * @return ResponseEntity with status 200 and SessionResponse containing
	 ...
	 * Extracts the most recently created session and formats response with all
	 * required fields.
	 *
