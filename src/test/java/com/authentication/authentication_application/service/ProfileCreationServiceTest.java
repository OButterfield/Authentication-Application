...
/**
 * Unit tests for ProfileCreationService. Tests user profile creation, password
...
class ProfileCreationServiceTest {
	...
	void shouldCreateUserProfileWithValidRequest() {
		// GIVEN
		User mockUser = User.builder().id(MONGO_ID).profileId(TEST_PROFILE_ID).email(TEST_EMAIL)
				.hashedPassword(HASHED_PASSWORD).createdAt(System.currentTimeMillis())
	...
	void shouldThrowDuplicateEmailExceptionForDuplicateEmail() {
		// GIVEN
		when(hashUtil.hash(TEST_PASSWORD)).thenReturn(HASHED_PASSWORD);
		when(userRepository.save(any(User.class))).thenThrow(new DuplicateKeyException(DUPLICATE_EMAIL_ERROR));
	...
	void shouldThrowDuplicateProfileExceptionForDuplicateProfileId() {
		// GIVEN
		when(hashUtil.hash(TEST_PASSWORD)).thenReturn(HASHED_PASSWORD);
		when(userRepository.save(any(User.class))).thenThrow(new DuplicateKeyException(DUPLICATE_PROFILE_ID_ERROR));

		// WHEN & THEN
		DuplicateProfileException exception = assertThrows(DuplicateProfileException.class,
				() -> profileCreationService.createUserProfile(validRequest));
	...
	void shouldThrowDuplicateProfileExceptionForUnknownDuplicateError() {
		// GIVEN
		when(hashUtil.hash(TEST_PASSWORD)).thenReturn(HASHED_PASSWORD);
		when(userRepository.save(any(User.class))).thenThrow(new DuplicateKeyException(GENERIC_DUPLICATE_ERROR));
