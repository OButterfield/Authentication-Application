...
/**
 * Integration tests for SessionsController. Tests the user login/session
 * creation endpoint with proper authentication. Cleans up created records in
...
class SessionsControllerTest {
	...
	void shouldCreateSessionWithAllRequiredFieldsAndValidFormats() throws Exception {
		...
		MvcResult result = mockMvc
				.perform(post("/sessions").contentType(MediaType.APPLICATION_JSON).content(sessionRequestBody))
				// THEN - Verify status, format, and all required fields
				.andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON))
				.andExpect(jsonPath("$.data").exists()).andExpect(jsonPath("$.data.sessionId", notNullValue()))
	...
	void shouldRejectSessionCreationWithInvalidEmailFormat() throws Exception {
		...
		// WHEN & THEN
		mockMvc.perform(post("/sessions").contentType(MediaType.APPLICATION_JSON).content(requestBody))
	...
	void shouldRejectSessionCreationWithIncorrectPassword() throws Exception {
		...
		// THEN
		mockMvc.perform(post("/sessions").contentType(MediaType.APPLICATION_JSON).content(sessionRequestBody))
				.andExpect(status().isUnauthorized()).andExpect(jsonPath("$.error").value("INVALID_CREDENTIALS"))
				.andExpect(jsonPath("$.message").value("Invalid email or password"));
	}
}
