...
/**
 * Service for handling user profile creation logic. Encapsulates user creation,
 * password hashing, and MongoDB persistence. Provides error handling for
...
public class ProfileCreationService {
	...
	/**
	 * Creates a new user profile from the creation request. Handles password
	 * hashing, unique ID generation, and MongoDB persistence.
	 *
	...
	/**
	 * Builds a User entity from the creation request. Generates unique profileId
	 * and timestamps.
	 *
	 * @param createProfileRequest
	 *            the request containing email and password
	 ...
	 */
	private User buildUserFromRequest(CreateProfileRequest createProfileRequest, String hashedPassword) {
		String profileId = UUID.randomUUID().toString();
		long now = System.currentTimeMillis();

		return User.builder().profileId(profileId).email(createProfileRequest.getEmail()).hashedPassword(hashedPassword)
				.createdAt(now).updatedAt(now).build();
	}

	/**
	 * Saves the user to MongoDB with error handling for duplicate email or
	 * profileId.
	 *
	 * @param user
	 ...
	 *             if email already exists (returns generic message for security)
	 * @throws DuplicateProfileException
	 *             if profileId already exists
	...
	private User saveUserToDatabase(User user) {
		try {
			...
			if (errorMessage != null && errorMessage.contains("email")) {
				...
				throw new DuplicateProfileException("DUPLICATE_PROFILE_ID", "profileId",
				...
				throw new DuplicateProfileException("DUPLICATE_ENTRY", "unknown",
						"A duplicate entry already exists (unknown field)");
			}
		} catch (Exception e) {
			logger.error("Failed to save user profile to database for email: {}. Error: {}", user.getEmail(),
					e.getMessage(), e);
			throw new AuthenticationApplicationException("DATABASE_ERROR",
					"Failed to save user profile. Please try again later.", e);
		}
	}
}
